...
CacheCore.set('x', {}); CacheCore.set('y', {}); CacheCore.set('z', {});
...
var h1 = computeCellHash('2026-06-15', { phase: 'period-on', annType: 0, isBirthday: false, markerCount: 0, diaryInfo: null, hasSymptom: false, solarTermName: '', lunar: null });
assert(typeof h1 === 'string', 'hash is string');
assert(h1.length > 5, 'hash has content');
var h2 = computeCellHash('2026-06-15', { phase: 'period-on', annType: 0, isBirthday: false, markerCount: 0, diaryInfo: null, hasSymptom: false, solarTermName: '', lunar: null });
assertEqual(h1, h2, 'same data -> same hash');
var h3 = computeCellHash('2026-06-15', { phase: 'luteal', annType: 0, isBirthday: false, markerCount: 0, diaryInfo: null, hasSymptom: false, solarTermName: '', lunar: null });
assert(h1 !== h3, 'different phase -> different hash');
var h5 = computeCellHash('2026-06-15', { phase: 'period-on', annType: 0, isBirthday: false, markerCount: 2, diaryInfo: null, hasSymptom: false, solarTermName: '', lunar: null });
assert(h1 !== h5, 'marker count -> different hash');
var h6 = computeCellHash('2026-06-15', { phase: 'period-on', annType: 0, isBirthday: false, markerCount: 0, diaryInfo: { hasAny: true }, hasSymptom: false, solarTermName: '', lunar: null });
assert(h1 !== h6, 'diary -> different hash');
var h7 = computeCellHash('2026-06-15', { phase: 'period-on', annType: 0, isBirthday: true, markerCount: 0, diaryInfo: null, hasSymptom: false, solarTermName: '', lunar: null });
...
var CalendarState = (function () {
  ...
  function getAll() { return { viewMonth: _state.viewMonth, viewYear: _state.viewYear, selectedDate: _state.selectedDate, knowledgeOpen: _state.knowledgeOpen, initialized: _state.initialized }; }
  function set(k, v, silent) { if (!(k in _state)) return; var old = _state[k]; if (old === v) return; _state[k] = v; if (!silent) { var s = _listeners[k]; if (s) s.forEach(function(fn) { fn(v, old); }); } }
  function batch(updates, silent) { var keys = Object.keys(updates); keys.forEach(function(k) { if (k in _state) _state[k] = updates[k]; }); if (!silent) keys.forEach(function(k) { var s = _listeners[k]; if (s) s.forEach(function(fn) { fn(_state[k], undefined); }); }); }
  function subscribe(k, fn) { if (!_listeners[k]) _listeners[k] = []; _listeners[k].push(fn); return function() { _listeners[k] = (_listeners[k]||[]).filter(function(f) { return f !== fn; }); }; }
...
CalendarState.set('viewMonth', 6); assertEqual(CalendarState.get('viewMonth'), 6, 'set changes viewMonth');
...
var notified = [];
var unsub = CalendarState.subscribe('viewMonth', function(n, o) { notified.push({ newVal: n, oldVal: o }); });
...
CalendarState.set('viewMonth', 5); CalendarState.set('knowledgeOpen', false);
var bn = [];
CalendarState.subscribe('viewMonth', function(n) { bn.push('m:'+n); });
CalendarState.subscribe('knowledgeOpen', function(n) { bn.push('k:'+n); });
...
var sn = [];
CalendarState.subscribe('viewMonth', function() { sn.push('x'); });
