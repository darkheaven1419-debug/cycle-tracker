...
const d=n.length>0?_median(n.slice(-PRED_DURATION_SAMPLES)):e.periodLength,
...
/* 未来列表（futurePeriods）从这一刻起有了两道闸：
   ...
   ② 窗口闸 —— 只留「今天 → 今天 + 3 个日历月」以内的。日期往后递推，
      一旦越过窗口就 break，所以不存在无限循环；PRED_MAX_LOOP 是 base<=0 时的保险。 */
const base=e.manualOverride?e.cycleLength:s.avgCycle,hor=s.forecast.horizonEnd;
...
if(s.forecast.reliable&&base>0){let st=s.nextStart,k=0;while(st&&k<PRED_MAX_LOOP){if(st>hor)break;s.futurePeriods.push({start:st,ovulation:addDays(st,-14),fertileStart:addDays(st,-17),fertileEnd:addDays(st,-11)}),st=addDays(st,base),k++}}
