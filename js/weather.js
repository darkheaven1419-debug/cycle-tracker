...
var WeatherModule = (function() {
  ...
  function annDateLove() { return typeof state !== 'undefined' && state.annDateLove ? state.annDateLove : '2023-10-18'; }
  function annDateMet() { return typeof state !== 'undefined' && state.annDateMet ? state.annDateMet : null; }
  ...
  function fetchWeather() {
    ...
    if (cached) {
      try { var d = JSON.parse(cached); renderWeather(d); } catch (e) { console.warn('[weather] Bad cached data'); }
    }
    // Refresh in background (6h cache)
    if (cached) {
      try { var d2 = JSON.parse(cached); if (Date.now() - d2.t < 21600000) return; } catch (e) { console.warn('[weather] Bad cache'); }
    }
    ...
    try {
      ...
      Promise.all([bj, ki]).then(function(r) {
        ...
        if (bjD && r[0].daily) { bjD.sunrise = r[0].daily.sunrise[0]; bjD.sunset = r[0].daily.sunset[0]; }
        var kiD = r[1] ? r[1].current : null;
        if (kiD && r[1].daily) { kiD.sunrise = r[1].daily.sunrise[0]; kiD.sunset = r[1].daily.sunset[0]; }
  ...
  function updateWeatherTimes() {
    ...
    if (diffEl) {
      var bjH = parseInt(bjT), kiH = parseInt(kiT);
      var diff = bjH - kiH;
      if (diff < 0) diff += 24;
  ...
  function renderWeather(w) {
    ...
    if (nh) {
      ...
      if (parseInt(kiH) >= 22 || parseInt(kiH) <= 5) {
        nh.style.display = '';
        nh.textContent = L('🌙 Kod tebe je kasno - vreme za spavanje, Anđela 🛏️', '🌙 Kikinda现在是深夜，Angie该休息了', '🌙 It\'s late in Kikinda — time for sleep, Anđela 🛏️');
      } else {
  ...
  function updateLoveCounter() {
    ...
    if (annDateMet()) {
      ...
      if (d >= 0) parts.push('<div style="font-size:.85rem"><span style="color:var(--gold)">✨</span> ' + d + (lang === 'sr' ? ' dana od prvog susreta' : lang === 'en' ? ' days since we met' : ' 天前初次相遇') + '</div>');
    }
    if (annDateLove()) {
      var d = daysDiff(new Date(annDateLove()), today());
      if (d >= 0) parts.push('<div style="font-size:1.2rem;font-weight:700;color:var(--love)">♥ ' + d + (lang === 'sr' ? ' dana zajedno' : lang === 'en' ? ' days together' : ' 天在一起') + '</div>');
    }
  ...
  function randomThinkingOfYou() {
    ...
    if (typeof toast === 'function') setTimeout(function() { toast(msg); }, 3000);
  }
