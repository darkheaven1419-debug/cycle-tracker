...
function animateLoginEntrance() {
  ...
  gsap.fromTo(cards,
    ...
    { y: 0, autoAlpha: 1, scale: 1, duration: 0.5, stagger: 0.12, ease: 'back.out(1.4)' }
  );
}
...
function animateGreetingIn() {
  ...
  gsap.fromTo(card,
    ...
    { scale: 1, autoAlpha: 1, y: 0, duration: 0.5, ease: 'back.out(1.7)' }
  );
}
...
function animateGreetingOut(el) {
  if (!GSAP_ENABLED) { el.classList.add('hidden'); return; }
  gsap.to(el, { autoAlpha: 0, scale: 0.95, duration: 0.25, ease: 'power2.in',
...
function animateCalendarDays() {
  ...
  gsap.to(days, {
    autoAlpha: 1, y: 0, duration: 0.3, stagger: { amount: 0.3, from: 'center' },
    ease: 'power1.out'
  });
}
...
function animateModalOut(modalEl) {
  if (!GSAP_ENABLED) { if (modalEl) modalEl.classList.add('hidden'); return; }
  var inner = modalEl ? modalEl.querySelector('.modal') : null;
  if (!inner) { if (modalEl) modalEl.classList.add('hidden'); return; }
  gsap.to(inner, {
...
function showToast(msg, type) {
  ...
  if (GSAP_ENABLED) {
    gsap.fromTo(toast,
      { y: 40, autoAlpha: 0 },
      { y: 0, autoAlpha: 1, duration: 0.35, ease: 'back.out(1.2)' }
    );
    gsap.to(toast, {
...
function animateProgressBar(fillEl, pct) {
  ...
  gsap.to(fillEl, {
    scaleX: pct / 100, duration: 0.6, ease: 'power2.out', transformOrigin: 'left center'
  });
}
