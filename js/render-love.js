...
function getHugStreak() {
  ...
  for (let i = 0; i < 365; i++) {
    ...
    if (day && day['barry'] && day['barry'].hug && day['andjela'] && day['andjela'].hug) { streak++; }
    else { break; }
  }
  return streak;
}
function sendHug(hugBack) {
  ...
  if (count >= 2) { toast(t('hugLimit')); return; }
  count++;
...
function dismissHug() { localStorage.removeItem('shared-hug'); renderHug(); }
function renderHug() {
  const hug = checkHug();
  ...
  if (hug) {
    ...
    html += '<div><button class="hug-dismiss" onclick="dismissHug()">' + (lang === 'sr' ? '✕ zatvori' : lang === 'en' ? '✕ dismiss' : '✕ 关闭') + '</button></div></div>';
    card.innerHTML = html;
    const hugCard = document.getElementById('hugCard');
    if (hugCard) spawnFloatingHearts(hugCard);
  } else if (count > 0) {
    let sentHearts = '';
    for (let i = 0; i < 2; i++) { sentHearts += '<span class="hh-heart' + (i >= remaining ? ' used' : '') + '">' + (i < count ? '❤️' : '🤍') + '</span>'; }
    var html = '<div class="hug-sent-state">' + '<div class="hug-hearts-row">' + sentHearts + '</div><span class="hss-icon">📬</span><div class="hss-text">' + t('hugSentWaiting') + '</div><button class="hug-back-btn" onclick="sendHug()" style="margin-top:8px">🤗 ' + (lang === 'sr' ? 'Pošalji još jedan (' + remaining + ')' : lang === 'en' ? 'Send another (' + remaining + ')' : '再抱一次 (' + remaining + ')') + '</button></div>';
    card.innerHTML = html;
  } else {
...
function addGratitude() {
  ...
  _gratNotes = null; input.value = ''; renderGratitude(); pushAllSharedData();
}
function renderGratitude() {
  ...
  list.innerHTML = notes.slice(-5).reverse().map(function (n, i) {
    ...
    const btnHtml = needTrans ? ' <button onclick="translateGrat(' + i + ')" style="font-size:.55rem;padding:1px 6px;border-radius:8px;border:1px solid var(--border);background:var(--card);color:var(--text);cursor:pointer">🌐</button>' : '';
...
function saveCheckinAnswer(qIdx, answer) {
  ...
  renderCheckin(); pushAllSharedData();
}
function getCheckinAnswers(profile) { return JSON.parse(localStorage.getItem('shared-checkin-' + profile) || '{}'); }
function renderCheckin() {
  ...
  let html = questions.map(function (q, i) {
    ...
    const optsHtml = q.opts.map(function (o) {
      return '<span class="cq-opt' + (myPick === o ? ' picked' : '') + '" onclick="saveCheckinAnswer(' + i + ",'" + o.replace(/'/g, "\\'") + '\')">' + o + '</span>';
    }).join('');
...
function saveMySong() {
  ...
  if (!title) { toast(t('songSaveEmpty')); return; }
  ...
  renderSong(); pushAllSharedData(); toast(t('songSaved'));
}
function loadSong(profile) { return safeParse(localStorage.getItem('shared-song-' + profile), null); }
...
function renderKnowMe() {
  ...
  if (myAns) {
    html += '<div style="background:var(--rose-light);border-radius:12px;padding:10px 14px;margin-bottom:8px"><span style="font-size:.62rem;color:var(--text-muted)">' + myName + ' ' + (lang === 'sr' ? 'odgovor' : lang === 'en' ? ' answer' : '的回答') + '</span><div style="font-size:.8rem;color:var(--text);margin-top:4px">' + esc(myAns.answer) + '</div></div>';
  } else {
  ...
  if (partnerAns) {
    html += '<div style="padding-top:8px;border-top:1px solid var(--border);margin-top:4px"><span style="font-size:.62rem;color:var(--teal);font-weight:600">👀 ' + partnerName + t('knowMePartnerLabel') + '</span><div style="font-size:.82rem;color:var(--teal);margin-top:4px;font-style:italic;line-height:1.4">' + esc(partnerAns.answer) + '</div></div>';
    if (myAns && partnerAns && myAns.answer.trim().toLowerCase() === partnerAns.answer.trim().toLowerCase()) {
      html += '<div style="text-align:center;margin-top:8px;font-size:1.5rem;animation:bounce-arrow .8s infinite">💞</div><div style="text-align:center;font-size:.7rem;color:var(--love);font-weight:600">' + t('knowMeMatch') + '</div>';
    }
  } else if (myAns) {
...
function saveKnowMeAnswer() {
  ...
  saveKnowMeData(allData); pushAllSharedData(); renderKnowMe();
...
function renderSong() {
  ...
  let html = '';
  if (mySong) {
    html += '<div style="margin-bottom:10px"><span style="font-size:.62rem;color:var(--text-muted)">' + t('songMyLabel') + '</span><div class="song-title">🎶 ' + esc(mySong.title) + '</div>' + (mySong.note ? '<div class="song-note">' + esc(mySong.note) + '</div>' : '') + '</div>';
  } else {
    html += '<div style="margin-bottom:10px"><input id="songInputTitle" placeholder="' + t('songTitlePlaceholder') + '" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:12px;font-size:.74rem;font-family:var(--font);background:var(--card);color:var(--text);margin-bottom:6px"><input id="songInputNote" placeholder="' + t('songNotePlaceholder') + '" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:12px;font-size:.74rem;font-family:var(--font);background:var(--card);color:var(--text);margin-bottom:6px"><button class="btn btn-primary" onclick="saveMySong()" style="width:100%;font-size:.7rem;padding:8px">🎵 ' + t('songSave') + '</button></div>';
  }
  if (partnerSong) {
    html += '<div style="padding-top:8px;border-top:1px solid var(--border)"><span style="font-size:.62rem;color:var(--text-muted)">' + partnerName + ' ' + t('songPartnerLabel') + '</span><div class="song-title">🎶 ' + esc(partnerSong.title) + '</div>' + (partnerSong.note ? '<div class="song-note">' + esc(partnerSong.note) + '</div>' : '') + '</div>';
  }
