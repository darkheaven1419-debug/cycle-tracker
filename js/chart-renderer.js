...
const ChartRenderer = {
  _theme: function () {
    ...
    return {
      ...
      line: isDark ? '#c47078' : '#b8485a',
      fill: isDark ? 'rgba(196,112,120,0.15)' : 'rgba(184,72,90,0.12)',
      dot: isDark ? '#c47078' : '#b8485a',
      fillEnd: isDark ? 'rgba(196,112,120,0.01)' : 'rgba(184,72,90,0.01)',
      sage: isDark ? '#8fc7b0' : '#80a590',
      teal: isDark ? '#7ab8a5' : '#5e8b7a',
      lavender: isDark ? '#c8b8d8' : '#b8a0c8',
      gold: isDark ? '#d4aa6e' : '#c49a5e',
      donutColors: [
        isDark ? '#c47078' : '#b8485a',
  ...
  drawLineChart: function (canvas, dataPoints, labels, opts) {
    ...
    const self = this;
    const t = self._theme();
    const _a = self._setupCanvas(canvas, opts.width || 500, opts.height || 200),
      ctx = _a.ctx,
      w = _a.w,
      h = _a.h;
    ...
    if (!dataPoints || dataPoints.length === 0) {
      ...
      ctx.font = 'italic .68rem ' + getComputedStyle(document.body).fontFamily;
    ...
    for (let i = 0; i <= gridLines; i++) {
      ...
      ctx.stroke();
      ctx.fillStyle = t.textMuted;
      ctx.font = '.55rem ' + getComputedStyle(document.body).fontFamily;
      ctx.textAlign = 'right';
      ctx.fillText(Math.round(maxVal - ((maxVal - minVal) / gridLines) * i), pad.left - 6, y + 3);
    }
    ctx.setLineDash([]);
    ...
    if (labels && labels.length > 0) {
      ctx.fillStyle = t.textMuted;
      ctx.font = '.52rem ' + getComputedStyle(document.body).fontFamily;
      ctx.textAlign = 'center';
    ...
    if (opts.avgLine) {
      ...
      ctx.font = '.52rem ' + getComputedStyle(document.body).fontFamily;
      ctx.textAlign = 'left';
      ctx.fillText(opts.avgLabel || 'Avg', w - pad.right - 24, ay - 4);
    }

    // Fill area
    const grad = ctx.createLinearGradient(0, pad.top, 0, pad.top + ph);
    grad.addColorStop(0, t.fill);
    grad.addColorStop(1, t.fillEnd);
    ctx.fillStyle = grad;
    ctx.beginPath();
    ...
    for (let i5 = 0; i5 < dataPoints.length; i5++) {
      const cx = pad.left + i5 * xStep,
        cy = valToY(dataPoints[i5]);
      ctx.beginPath();
      ...
      ctx.font = 'bold .55rem ' + getComputedStyle(document.body).fontFamily;
      ctx.textAlign = 'center';
      ctx.fillText(dataPoints[i5], cx, cy - 10);
    }
  },
  ...
  drawDonutChart: function (canvas, segments, opts) {
    ...
    const self = this;
    const t = self._theme();
    const _a = self._setupCanvas(canvas, opts.width || 260, opts.height || 200),
      ctx = _a.ctx,
      w = _a.w,
      h = _a.h;
    const cx = w / 2,
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (sliceAngle > 0.35 && segments[i].value > 0) {
        ctx.fillStyle = t.text;
        ctx.font = 'bold .52rem ' + getComputedStyle(document.body).fontFamily;
        ctx.textAlign = 'center';
    ...
    // Center text
    ctx.fillStyle = t.text;
    ctx.font = 'bold .9rem ' + getComputedStyle(document.body).fontFamily;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
  ...
  drawBarChart: function (canvas, bars, opts) {
    ...
    const self = this;
    const t = self._theme();
    const _a = self._setupCanvas(canvas, opts.width || 460, opts.height || 200),
      ctx = _a.ctx,
      w = _a.w,
      h = _a.h;
    let maxVal = 1;
    for (let bi = 0; bi < bars.length; bi++) {
      if (bars[bi].value > maxVal) maxVal = bars[bi].value;
    }
    ...
    for (let i = 0; i < bars.length; i++) {
      ...
      // Label
      ctx.fillStyle = t.text;
      ctx.font = '.6rem ' + getComputedStyle(document.body).fontFamily;
      ctx.textAlign = 'right';
      ctx.fillText(bars[i].label, labelW - 6, y + barH / 2 + 3);

      // Bar bg
      ctx.fillStyle = t.grid;
      ChartRenderer._roundRect(ctx, labelW + 4, y, barAreaW, barH, 4);
      ctx.fill();

      // Bar fill
      ctx.fillStyle = bars[i].color || t.line;
      ChartRenderer._roundRect(ctx, labelW + 4, y, bw, barH, 4);
      ctx.fill();

      // Value
      ctx.fillStyle = t.text;
      ctx.font = 'bold .58rem ' + getComputedStyle(document.body).fontFamily;
      ctx.textAlign = 'left';
      ctx.fillText(bars[i].value, labelW + bw + 10, y + barH / 2 + 3);
    }
  },
  ...
  drawSparkline: function (canvas, dataPoints, opts) {
    ...
    const self = this;
    const t = self._theme();
    const _a = self._setupCanvas(canvas, opts.width || 120, opts.height || 36),
      ctx = _a.ctx,
      w = _a.w,
      h = _a.h;
    ctx.clearRect(0, 0, w, h);
    ...
    if (!dataPoints || dataPoints.length < 2) {
      ctx.fillStyle = t.textMuted;
      ctx.font = '.5rem ' + getComputedStyle(document.body).fontFamily;
      ctx.textAlign = 'center';
      ctx.fillText('--', w / 2, h / 2 + 4);
      return;
    }

    const minV = Math.min.apply(Math, dataPoints),
      maxV = Math.max.apply(Math, dataPoints);
    ...
    // Last dot
    const lx = pad + (dataPoints.length - 1) * xStep,
      ly = valToY(dataPoints[dataPoints.length - 1]);
    ctx.beginPath();
