...
(function() {
  ...
  window.selectLogin = function(p) {
    selected = p;
    $('loginCardAndjela').classList.toggle('selected', p === 'andjela');
    $('loginCardBarry').classList.toggle('selected', p === 'barry');
    $('loginPinArea').classList.add('show');
    $('loginPinInput').value = '';
    $('loginError').textContent = '';
    setTimeout(function() { $('loginPinInput').focus(); }, 300);
  };

  window.verifyLogin = function() {
    var pin = $('loginPinInput').value;
    if (pin === PIN[selected]) {
      profile = selected;
      ...
      $('loginError').textContent = 'Pogresan PIN';
      $('loginPinInput').value = '';
    }
  };
  ...
  function showApp() {
    ...
    $('sb').addEventListener('click', saveD);
    $('mb').addEventListener('click', saveM);
    $('lb').addEventListener('click', function() { sessionStorage.removeItem('cycle-logged-in'); location.reload(); });
    renderD(); renderM();
  }

  function loadD() { try { return JSON.parse(localStorage.getItem('es5-diary')) || {}; } catch(e) { return {}; } }
  function saveD() {
    var tx = $('da').value.trim(); if (!tx) return;
    var d = loadD(), k = fmt();
  ...
  function saveM() {
    var tx = $('ma').value.trim(); if (!tx) return;
  ...
  // Init
  if (sessionStorage.getItem('cycle-logged-in') === '1') { $('loginOverlay').classList.add('hidden'); showApp(); }
})();
